...
class GroupControllerTest extends TestCase
{
    ...
    public function testGroupDetach(): void
    {
        ...
        $numGroup = $group->workers()->count();
        /** @var Worker $worker */
        $worker = $group->workers()->inRandomOrder()->first();
        $numWorkers = $worker->groups()->count();
        $response = $this->actingAs($this->users['admin'])->delete(route('group.detach'), ['group_id' => $group->id, 'worker_id' => $worker->id], ['Accept' => 'application/json']);
        $response->assertStatus(ResponseCode::HTTP_NO_CONTENT);
